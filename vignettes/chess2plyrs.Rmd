...
The `style` option allows for different piece styles:
 ...
 - `style = 2` plots pieces with their labels instead of the unicode. As unicode may yield an error on some platforms, in this vignette we use this option
...
The function `game_result` tells whether the game is still ongoing or if there is a checkmate or stalemate. Note that up to now threefold repetition is not implemented as a recognised draw situation yet.
...
Along with a simple random mover engine, a slightly more sophisticated engine which uses minimax and alpha-beta pruning is present in this package. 
For more information have a look at https://www.r-bloggers.com/2022/07/programming-a-simple-minimax-chess-engine-in-r/
...
```{r}
#engine2(ck4, 1) # "pa6b5" is the chosen move
#engine2(ck4, 2) # "pa6b5" is the chosen move
#engine2(ck4, 3) # "Qd8h4" is the chosen move
```
...
1) pawn promotion is automatically to Queen at the moment. In a future release, autopromotion will be removed
...
4) Draw by three-fold repetition is currently not recognised
